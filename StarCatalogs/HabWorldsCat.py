...
class StarCatalog():
    ...
    def __init__(self,
                 ...
                 Dist_range=[0, 30.],  # pc
                 RA_range=[0., 360.],  # deg
                 Path='/Users/fdannert/Documents/projects/LIFEsim/working/habitable_worlds_observatory/hwo_catalog/hwo_catalog_rt.csv'):
        """
        Parameters
        ----------
        Stypes: list
    ...
    def read(self,
        ...
        # Print.
        print('--> Reading star catalog hwo_catalog')
        ...
        with open(Path) as csvfile:
            ...
            for i, row in enumerate(SC_in):
                if (i == 0):
                    ...
                    #
                    row = np.array(row)
                    ColName = np.where(row == 'Name')[0][0]
                    ColDist = np.where(row == 'dist')[0][0]
                    ColStype = np.where(row == 'Spec_simple')[0][0]
                    ColRad = np.where(row == 'radius')[0][0]
                    ColTeff = np.where(row == 'teff')[0][0]
                    ColMass = np.where(row == 'mass')[0][0]
...
#                    ColWDSdmag = np.where(row == 'wds_deltamag')[0][0]
                    CollGal = np.where(row == 'lat')[0][0]
                    ColbGal = np.where(row == 'lon')[0][0]
